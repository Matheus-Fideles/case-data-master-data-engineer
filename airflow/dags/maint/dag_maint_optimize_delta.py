"""Maintenance DAG — OPTIMIZE + VACUUM on Delta Lake tables.

Runs weekly (Sunday 03:00) to:
  1. Compact small files via OPTIMIZE (improves analytical reads)
  2. ZORDER on high-cardinality fact columns (reduces data skipping)
  3. VACUUM to remove old versions and comply with LGPD retention

Retention by layer (ADR-0004 / LGPD Art. 16):
  - Bronze: 7 days (168h)  — raw data with PII, retain minimum
...
def _optimize_tables(paths: list[str], label: str, **context) -> dict:
    """Runs OPTIMIZE on each Delta table in the list."""
    spark = _get_spark()
    results = {}
    for path in paths:
        try:
            ...
            log.info("[%s] OPTIMIZE complete: %s → %s", label, path, metrics)
        except Exception as e:
            log.error("[%s] OPTIMIZE failed for %s: %s", label, path, e)
    ...
    if errors:
        raise RuntimeError(f"OPTIMIZE failed on {len(errors)} table(s): {errors}")
    return results
...
def _optimize_gold_zorder(**context) -> dict:
    """OPTIMIZE with ZORDER on high-cardinality Gold fact columns."""
    spark = _get_spark()
    results = {}
    for path, zorder_cols in _GOLD_TABLES:
        try:
            ...
            log.error("[gold] OPTIMIZE failed for %s: %s", path, e)
            results[path] = {"status": "error", "error": str(e)}

    context["ti"].xcom_push(key="optimize_gold_results", value=results)
    errors = [p for p, r in results.items() if r["status"] == "error"]
    if errors:
        raise RuntimeError(f"OPTIMIZE Gold failed on {len(errors)} table(s): {errors}")
    return results


def _vacuum_bronze(**context) -> dict:
    """VACUUM Bronze with minimum retention (raw data with PII — LGPD)."""
    spark = _get_spark()
    ...
    for path in _BRONZE_TABLES:
        try:
            ...
            log.error("[vacuum/bronze] failed %s: %s", path, e)
...
def _vacuum_silver_gold(**context) -> dict:
    """VACUUM Silver and Gold with operational retention (30 days)."""
    spark = _get_spark()
    spark.conf.set("spark.databricks.delta.retentionDurationCheck.enabled", "false")
    results = {}
...
with DAG(
    ...
    # Bronze and Silver in parallel → Gold → VACUUM Bronze → VACUUM Silver/Gold
